...
// =========================== Get NFT Minter in OGY Standard ===========================
...
export const queryTokenListingByOgy = async (
    ...
    return list.map((d, index) => {
        ...
        if (current_sale === undefined) return listing_data; // Not listed if there is no sale information
        listing_data.listing = unwrapVariant1Map<AuctionStateShared, NftListing>(
            current_sale.sale_type,
            [
                'auction',
                (a) =>
                    parseOgyAuctionToNftListing_47a7c018(current_sale.sale_id, a, yuku_ogy_broker),
            ],
        );
        return listing_data;
    });
        ...
        sales_config: {
            ...
            pricing: {
                auction: {
                    ...
                    start_price: string2bigint(args.price), // start_price and buy_now are set the same for immediate purchase
...
export const queryRechargeAccountByOgy = async (
    identity: ConnectedIdentity,
    collection: string,
    principal: string, // Buyer's principal
): Promise<string> => {
    const { creator } = identity;
    const actor: _SERVICE = await creator(idlFactory, collection);
...
// =========================== Batch Purchase NFT in OGY Standard ===========================
...
// =========================== Query All Token History Records in OGY Standard ===========================
