...
export const BACKEND_TYPE = '__yuku_backend__';

const LAST_CONNECT_TYPE = '__yuku_last_connect_type__';
...
const LAST_CONNECT_EMAIL = '__yuku_last_connect_email__';
...
const LAST_USED_TYPE = '__yuku_last_used_type__';
export const readLastUsedType = () => readStorage(LAST_USED_TYPE) ?? '';
export const writeLastUsedType = (connectType: ConnectType | '' | 'email') =>
    writeStorage(LAST_USED_TYPE, connectType);

export const UNITY_USER_TOKEN = '__unity_user_token__';

export const YUKU_USER_PERMISSIONS = '__yuku_user_permissions__';
...
export const writeIcpUsdByDate = (date: string, price: number) => {
    ...
    writeStorage(
        ICP_USD_HISTORY,
        old
            ? JSON.stringify({ ...JSON.parse(old), [date]: price })
            : JSON.stringify({ [date]: price }),
    );
};
